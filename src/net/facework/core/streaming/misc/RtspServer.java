...
public class RtspServer extends Service {
	...
	public static String SERVER_NAME = "face RTSP Server";
	...
	public void start() {
		Log.i(TAG,"RTSP server is starting...");
		if (mRestart) stop();
		if (mEnabled && mListenerThread == null) {
			try {
				mListenerThread = new RequestListener();
				Log.i(TAG,"RTSP server is started.");
			} catch (Exception e) {
				mListenerThread = null;
				Log.i(TAG,"RTSP server is failed to create RequestListener");
			}
		}
		mRestart = false;
	}
	...
	public void stop() {
		Log.i(TAG,"RTSP server is stopping...");
		if (mListenerThread != null) {
			try {
				mListenerThread.kill();
				Log.i(TAG,"RTSP server is stopped...");
			} catch (Exception e) {
			} finally {
				mListenerThread = null;
			}
		...
		public Response processRequest(Request request) throws IllegalStateException, IOException {
			...
			if (request.method.toUpperCase().equals("DESCRIBE")) {
				...
				String requestAttributes = 
						"Content-Base: rtsp://"+mClient.getLocalAddress().getHostAddress()+":"+mClient.getLocalPort()+"/"+UriParser.filename+"/\r\n" +
			...
			/* ********************************************************************************** */
				...
				if (mSession.trackExists(0)) requestAttributes += "url=rtsp://"+mClient.getLocalAddress().getHostAddress()+":"+mClient.getLocalPort()+"/"+UriParser.filename+"/trackID="+0+";seq=0;rtptime=0,";
				if (mSession.trackExists(1)) requestAttributes += "url=rtsp://"+mClient.getLocalAddress().getHostAddress()+":"+mClient.getLocalPort()+"/"+UriParser.filename+"/trackID="+1+";seq=0;rtptime=0,";
			...
			/* ********************************************************************************** */
			else if (request.method.toUpperCase().equals("GET_PARAMETER")) {
				response.status = Response.STATUS_OK;// this method used as heartbeat;
			}
	...
	static class Request {
		...
		public static Request parseRequest(BufferedReader input) throws IOException, IllegalStateException, SocketException {
			...
			Log.d(TAG,request.method+" "+request.uri+" "+request.headers);
	...
	static class Response {
		...
		public void send(OutputStream output) throws IOException {
			...
			try {
				Log.i(TAG," "+mRequest.headers.get("cseq"));
