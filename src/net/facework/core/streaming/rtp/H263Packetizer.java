...
public class H263Packetizer extends AbstractPacketizer implements Runnable {
		...
		try { 
			while (running) {
				...
				if (j>0) {
					...
					socket.send(j);
					Log.d(TAG,"send out a frame Size:"+j);
					...
					Log.d(TAG,"send out a fragment of a frame, j:"+j);
				}
			}
		} catch (IOException e) {
			running = false;
	...
	private int fill(int offset,int length) throws IOException {
		...
		{
			if(this.readSize+length>this.mdatSize)
			{
				length=this.mdatSize-this.readSize;
				running = false;
			}
		}
		while (sum<length) {
	...
	private void skipHeader() throws IOException {
		...
		{
			byte skip[] = null;
			is.read(buffer,rtphl,4);
			int ftypSize=buffer[rtphl+3]&0xFF | (buffer[rtphl+2]&0xFF)<<8 | (buffer[rtphl+1]&0xFF)<<16 | (buffer[rtphl]&0xFF)<<24;
			Log.i(TAG,"ftypSize :"+ftypSize);
			for(int i=0;i<ftypSize;i++)
			{
				is.read();
			}
			is.read(buffer,rtphl,4);
			this.mdatSize=buffer[rtphl+3]&0xFF | (buffer[rtphl+2]&0xFF)<<8 | (buffer[rtphl+1]&0xFF)<<16 | (buffer[rtphl]&0xFF)<<24;
			Log.i(TAG,"mdat Size:"+mdatSize);
			is.read(buffer,rtphl+3,4);
			if(buffer[rtphl+3]=='m' && buffer[rtphl+4] == 'd' && buffer[rtphl+5] == 'a' && buffer[rtphl+6] == 't');
		}
		else
			while (true) {
				while (is.read() != 'm');
