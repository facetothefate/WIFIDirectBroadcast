...
public class H264Packetizer extends AbstractPacketizer implements Runnable{
	...
	private void send() throws IOException, InterruptedException {
		...
		//Log.i(TAG,"NAL Undddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddit length byte:"+toHexString(buffer,rtphl,4));
		//Log.i(TAG,"NAL Undddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddit length byte:"+toHexString(buffer,rtphl,4));
		//Log.i(TAG,"NAL Undddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddit length byte:"+toHexString(buffer,rtphl,4));
		...
		//Log.i(TAG,"type:"+type);
		if(type==(byte) Integer.parseInt("00011110",2))
		{
			fill(rtphl,4);
			ts=buffer[rtphl+3]&0xFF | (buffer[rtphl+2]&0xFF)<<8 | (buffer[rtphl+1]&0xFF)<<16 | (buffer[rtphl]&0xFF)<<24;
		...
		else if (naluLength<=MAXPACKETSIZE-rtphl-2) {
			
			len = fill(rtphl+1,  naluLength-1  );
			socket.markNextPacket();
			//socket.setType(buffer[rtphl]);
