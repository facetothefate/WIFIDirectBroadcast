...
import net.facework.core.streaming.audio.AACFileStream;
import net.facework.core.streaming.audio.AACStream;
import net.facework.core.streaming.audio.AMRNBStream;
import net.facework.core.streaming.video.H263FileStream;
import net.facework.core.streaming.video.H263Stream;
import net.facework.core.streaming.video.H264FileStream;
import net.facework.core.streaming.video.H264Stream;
import net.facework.core.streaming.video.VideoQuality;
import net.facework.core.streaming.video.VideoStream;
...
public class Session {
	...
	public final static int FILE_AUDIO_AAC = 0x11;
	...
	/** Can be used with addVideoTrack to add an H.264 file */
	public final static int FILE_VIDEO_H264 = 0x06;
	public final static int FILE_VIDEO_H263 = 0x07;
	...
	public void setFile(String name){
		Log.i(TAG,"RTSP server is openning "+Environment.getExternalStorageDirectory().getPath()+"/"+name);
		this.f=new File(Environment.getExternalStorageDirectory().getPath()+"/"+name);
		mp4=new MP4Info(this.f);
		mp4.getInfo();
	}
	 ...
	 * @throws IOException
	 */
	public Session addFileVideoTrack(int encoder){
		
		
		FileMediaStream stream=null;
		switch (encoder) {
		case FILE_VIDEO_H264:
			stream = new H264FileStream(f);
			Log.d(TAG,"FILE Video streaming: H.264");
			break;
		case FILE_VIDEO_H263:
			try {
				stream = new H263FileStream(f);
			} catch (IOException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			Log.d(TAG,"FILE Video streaming: H.263");
			break;
		}
		try {
	...
	public Session addFileAudioTrack(int encoder){
		FileMediaStream stream=null;
		switch (encoder) {
		case FILE_AUDIO_AAC:
			try {
				stream = new AACFileStream();
			} catch (IOException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			Log.d(TAG,"FILE Audio streaming: AAC");
			break;
		}
		try {
			stream.setTimeToLive(mDefaultTimeToLive);
	...
	public void startAll() throws IllegalStateException, IOException {
		Log.d(TAG,"stream length:"+mStreamList.length);
		for (int i=0;i<mStreamList.length;i++) {
			start(i);
			Log.d(TAG,"start track:"+i);
		}
	}
